...
package body System.Secondary_Stack is
   ...
   is
      ...
      Adjusted_Alignment : constant SSE.Storage_Count :=
         SSE.Storage_Count'Max (Alignment, Standard'Maximum_Alignment);
      --  Adjust the requested alignment to be at least
      --  Standard'Maximum_Alignment. If the caller requests a larger alignment
      --  use that instead.

      function Padding (Addr : Address) return SSE.Storage_Count;
      pragma Inline (Padding);
      --  Get the padding required to pad an address to Adjusted_Alignment

      function Padding (Addr : Address) return SSE.Storage_Count is
      begin
         return (Adjusted_Alignment - (Addr mod Adjusted_Alignment))
                  mod Adjusted_Alignment;
      end Padding;
      ...
      Top_Padding : constant SSE.Storage_Count :=
         Padding (Stack.Internal_Chunk (Stack.Top)'Address);

      Aligned_Top : SS_Ptr;

   begin

      --  Check that Adjusted_Top does not overflow SS_Ptr

      if SSE.Storage_Count (SS_Ptr'Last) - Top_Padding
         < SSE.Storage_Count (Stack.Top)
      then
         raise Storage_Error;
      end if;

      Aligned_Top := Stack.Top + SS_Ptr (Top_Padding);
      ...
      --  T : Stack.Top
      --  A : Aligned_Top
      --  L : Capability lower bound and allocated address (Addr)
      --  S : Addr + Storage_Size
      ...
      Adjusted_Storage_Size :=
        CHERI.Representable_Length
          (CHERI.Bounds_Length (Storage_Size));
      ...
      Addr := Stack.Internal_Chunk'Address +
        SSE.Storage_Offset (Aligned_Top - Stack.Internal_Chunk'First);

      Capability_Lower_Bound :=
        CHERI.Capability_With_Address_Aligned_Up (Addr, Adjusted_Storage_Size);

      Capability_Lower_Bound_Padding :=
        CHERI.Bounds_Length
          (SSE.Storage_Offset'(Capability_Lower_Bound - Addr));

      Aligned_Top := Aligned_Top + SS_Ptr (Capability_Lower_Bound_Padding);

      if CHERI.Bounds_Length (SS_Ptr'Last - Aligned_Top)
         < Adjusted_Storage_Size
      then
         raise Storage_Error;
      end if;
      ...
      if CHERI.Bounds_Length (Stack.Max) - CHERI.Bounds_Length (Aligned_Top)
         < Adjusted_Storage_Size
      then
         ...
         Free_Space := (Stack.Size - Aligned_Top) + 1;

         if CHERI.Bounds_Length (Free_Space) < Adjusted_Storage_Size then
            raise Storage_Error;
         end if;

         --  Record new max usage

         Stack.Max := Aligned_Top + SS_Ptr (Adjusted_Storage_Size);
      end if;
      ...
      Stack.Top := Aligned_Top + SS_Ptr (Adjusted_Storage_Size);
   end SS_Allocate;
